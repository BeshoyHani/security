...
namespace SecurityLibrary {
    ...
    class Matrix {
        ...
        private int getKey(int x, int res) {
            for (int i = 0; i < 26; i++) {
                if (i * x % 26 == res)
                    return i;
            }
        ...
        public List<int> Gauss(int[,] mat) {
            int n = mat.GetLength(0);
            int m = mat.GetLength(1) - 1;
            ...
            for (int col = 0, row = 0; col < m && row < n; col++) {
                ...
                for (int i = row; i < n; i++) {
                    if (mat[i, col] != 0) {
                        for (int j = col; j <= m; j++) {
                            swap(ref mat[i, j], ref mat[row, j]);
                        }
                        where[col] = row;
                        break;
                    }
                ...
                for (int i = 0; i < n; ++i) {
                    if (i != row && mat[i, col] != 0) {
                        int x = mat[row, col], y = mat[i, col];

                        for (int j = col; j <= m; ++j) {
                            mat[i, j] = x * mat[i, j] - y * mat[row, j];
            ...
            for (int i = 0; i < m; ++i) {
                if (where[i] == -1) res.Add(0);
                else {
                    int k = getKey(mat[where[i], i], mat[where[i], m]);
                    if (k == -1) throw new InvalidAnlysisException();
                    res.Add(k);
                }
            }
            ...
            for (int i = 0; i < n; ++i) {
                int sum = 0;
                for (int j = 0; j < m; ++j)
                    sum = (sum + res[j] * mat[i, j]) % 26;

                if (sum != mat[i, m]) throw new InvalidAnlysisException();
            }
    ...
    public class HillCipher : ICryptographicTechnique<List<int>, List<int>> {
        ...
        public List<int> Analyse(List<int> plainText, List<int> cipherText) {
            int[,] mat = new int[plainText.Count, 5];

            for (int c = 0; c < cipherText.Count; c++) {
                mat[c, 4] = cipherText[c];

                for (int i = 0; i < 4; i++) {
                    if (c % 2 == i / 2) {
                        mat[c, i] = plainText[c - c % 2 + i % 2];
                    }
                    else {
                        mat[c, i] = 0;
                    }
